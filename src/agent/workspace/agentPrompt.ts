...
const MOBILE_WORKSPACE_CONTRACT = [
  ...
  'Attachments are represented by bounded manifests, never by automatically inlining entire files. For CSV, TSV or JSONL, call table_inspect and table_query; use table_export only for a user-facing result.',
  'For office documents, PDFs, RTF, EPUB and OpenDocument files, use document_inspect, document_search and document_read. Local anydoc conversion is derived data and remains private unless document_export is explicitly needed.',
  'Do not use read on a whole large table or document. Inspect first, then request only the rows, sections or search matches needed for the task.',
